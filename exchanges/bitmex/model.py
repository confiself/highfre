...
def get_abnormal_minute(start, end):
    ...
    _root_path = '/opt/app/data/bitmex'
    ...
    while date_start <= date_end:
        ...
        with open(file_path) as f:
            ...
            # pm.bar(data_1_minute['t'][:100], data_1_minute['v'][:100], bottom=0.8, facecolor="#9999ff", edgecolor="white")
            abnormal_t_list = []
            skip_index = 10
            ...
            for i, v in enumerate(data_1_minute['v']):
                if i < skip_index:
                    continue
                if i > len(data_1_minute['t']) - 10:
                    break
                average = np.mean(data_1_minute['v'][i-10:i])
                max_value = max(data_1_minute['v'][i - 10:i])
                current_v = data_1_minute['v'][i]
                if current_v > 2 * average and max_value < current_v * 2 / 3:
                    state = _get_first_trigger(i)

                    print('find it', data_1_minute['t'][i], state)
                    skip_index = i + 10
                    # abnormal_t_list.append(data_1_minute['t'][i])
            # pm.plot(abnormal_t_list, [100000] * len(abnormal_t_list), '*')
            # pm.show()
        print(oc_dict)
...
if __name__ == '__main__':
    # plot_data()
    get_abnormal_minute('2018-02-12', '2018-02-13')